export type BathCue='fill'|'steam'|'drain'|'splat'|'sizzle';
/** Original quiet water, steam and comically low drain bubbles. */
export function bathSound(audio:AudioContext,cue:BathCue){
 const duration=cue==='drain'?6:cue==='sizzle'?1.8:cue==='steam'?2:cue==='fill'?2.4:.45;
 ...
 for(let i=0;i<d.length;i++){const t=i/audio.sampleRate,u=t/duration,n=Math.random()*2-1;low=low*.92+n*.08;const bubbles=cue==='drain'?Math.sin(t*(170-80*u)+Math.sin(t*17)*3)*Math.pow(Math.max(0,Math.sin(t*19)),4)*.35:0;d[i]=(low+bubbles+n*(cue==='sizzle'?.38:cue==='steam'?.06:.01))*Math.min(1,t/.1)*Math.pow(1-u,cue==='drain'?.4:1.4);}
