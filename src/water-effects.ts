  ...
  update(time:number,reduced:boolean){
    ...
    this.bursts.forEach((b,n)=>{
      for(let j=0;j<PARTICLES_PER_BURST;j++){
        const ring=Math.floor(j/12),age=time-b.at-(b.soil?0:ring*.045),life=b.soil?.42:.48;
        const visible=age>=0&&age<life;
        const a=(j%12)*Math.PI/6+n*.7+ring*Math.PI/12,speed=.8+(j%3)*.25;
        if(b.soil){
          // Six irregular pebbles per side, distributed around the trench lip.
          const edge=Math.floor(j/6),along=(j%6-2.5)*.25+Math.sin(n+j)*.07;
          ...
          this.dummy.scale.setScalar(fade*2);
          ...
          this.dummy.scale.setScalar(scale*(b.impact?2:1));
