...
export class BridgeRiver {
  ...
  constructor(world:World,parent:T.Group){
    ...
    for(let n=0;n<3;n++){
      ...
      ring.rotation.x=-Math.PI/2;ring.position.set(n===2?3.8:2.5,-6.05,n===2?3.7:5.1);ring.userData.ownedMaterial=ring.material;this.root.add(ring);this.rings.push(ring);
    }
  }
  update(time:number,storyTime:number,wheel:T.Group,runaway:number,reduced:boolean){
    ...
    for(let n=0;n<48;n++){
      ...
      if(n<24&&runaway>0&&runaway<1){
        ...
        const impact=n<36?6.45:9.45,age=storyTime-impact;
        if(age>0&&age<1.05){
          const angle=n*2.399,speed=.6+(n%4)*.25;
          this.dummy.position.set((n<36?2.5:3.8)+Math.cos(angle)*age*speed,-6.05+age*(2+(n%3)*.35)-age*age*3.5,(n<36?5.1:3.7)+Math.sin(angle)*age*speed);
    ...
    this.rings.forEach((ring,n)=>{
      const age=storyTime-[6.45,6.7,9.45][n];ring.visible=!reduced&&age>0&&age<1.1;
