import type {BathCue} from './bathhouse-sound';
...
export class World {
  ...
  mat(color: number, surface?: Surface, vertexColors = false) {
    const metric=["bridge","city","harvest","caravan","bathhouse"].includes(this.game.level.story?.kind ?? "") && (surface === "wood" || surface === "stone" || surface === "plaster");
  ...
  shaded(geometry: T.BufferGeometry, color: number, surface?: Surface) {
    if(["bridge","city","harvest","caravan","bathhouse"].includes(this.game.level.story?.kind ?? "")) {
  ...
  box(
    ...
    if(["bridge","city","harvest","caravan","bathhouse"].includes(this.game.level.story?.kind ?? "") && (surface === "wood" || surface === "stone" || surface === "plaster")) {
  ...
  build() {
    ...
    for (let i = 0; i < CELL_COUNT; i++) {
      ...
      if (t === "rock" && !["oasis", "bridge", "city", "harvest", "caravan","bathhouse"].includes(this.game.level.story?.kind ?? "")) {
    ...
    if (!["oasis","city","harvest","caravan","bathhouse"].includes(this.game.level.story?.kind ?? ""))
    ...
    if (["oasis", "bridge", "city", "harvest", "caravan","bathhouse"].includes(this.game.level.story?.kind ?? "")) { this.sync(); return; }
  ...
  machine(i: number, n: number) {
    if(["bridge","city","harvest","caravan","bathhouse"].includes(this.game.level.story?.kind ?? "")) {this.bridgeMachine(i);return;}
  ...
  disposeRoot() {
    ...
    this.story?.bathhouseScene?.dispose();
